...
class ffmpegRecipe(ConanFile):
    ...
    def generate(self):
        ...
        cc = tools_path + "/aarch64-linux-android%s-clang" % str(self.settings.os.api_level)
        cxx = tools_path + "/aarch64-linux-android%s-clang++" % str(self.settings.os.api_level)
        nm = tools_path + "/llvm-nm"
        llvm_ar = tools_path + "/llvm-ar"
        at_toolchain.configure_args.extend(["--enable-small", "--disable-programs", "--disable-avdevice",
                                       "--disable-doc", "--disable-symver",  "--enable-gpl",
                                       "--enable-pic", "--disable-debug",
                                       "--disable-postproc", "--enable-neon", "--enable-jni",
                                       "--disable-x86asm", "--enable-cross-compile", "--disable-stripping",
                                       "--cpu=armv8-a",
                                       "--cc=%s" % cc,
                                       "--cxx=%s" % cxx,
                                       "--nm=%s" % nm,
        ...
        if self.settings.build_type == "Debug":
            at_toolchain.configure_args.append("--enable-debug")
